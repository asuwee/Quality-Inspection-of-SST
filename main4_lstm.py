...
def train_process():
    ...
    test_loader = DataLoader(dataset=testSet, batch_size=256, shuffle=False)
    # 创建网络
    network = LSTMNetwork(8, 256, 1).to(device)
    loss_fun = nn.MSELoss()
    ...
    best_acc_P = 0
    # 保存日志
    with open('historys/m4_history.csv', 'a+', encoding='utf-8', newline='') as file:
    ...
    for epoch in range(epochs):
        ...
        if acc_P > best_acc_P:
            best_acc_P = acc_P
            network.save(f'weights/m4_weight.pt')
        # 保存日志
        with open('historys/m4_history.csv', 'a+', encoding='utf-8', newline='') as file:
            csv_writer = csv.writer(file)
